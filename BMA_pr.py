...
def RunBMA(obs, MOD):
    ...
    #Prepare data
    obs_data = obs_data.compressed().reshape((length_train, -1)) #360 * 18000
...
def PlotWeights(w):
    ...
    plt.ylabel("Weights")
    plt.savefig("Weights.png", format="png", dpi=600)
    plt.close()
    return
...
def PlotModels(obs_data, dataset_BMA):
    ...
    plt.ylabel("Pr($kg/m^2/s$)")
    plt.savefig("SpatialMean.png", format="png", dpi=600)
    plt.close()
    return
...
def CalculateRMSE(obs_data, dataset_BMA):
    ...
    RMSE = np.array(RMSE)
    np.savetxt("RMSE.txt", RMSE, delimiter=",")
    return RMSE

def PlotDensity(obs_data, dataset_BMA):
    bw = 0.2
    obs_mean = np.average(obs_data, axis=1)[:, np.newaxis] * 1e5
    SpatialMean = np.average(dataset_BMA, axis=2) * 1e5
    #X_plot = np.linspace(0, np.amax(np.vstack((obs_mean[:, 0], SpatialMean)))*2, 1000)[:, np.newaxis]
    X_plot = np.linspace(1.5, 3.5, 1000)[:, np.newaxis]
    plt.figure(figsize=(12, 8))
    for i in range(SpatialMean.shape[0]):
    ...
    plt.legend(loc="best")
    plt.xlabel("Precipitaton($Kg/m^2/s$)")
    plt.ylabel("PDF")
    plt.savefig("Density.png", format="png", dpi=600)            
    plt.close()
...
if __name__ == "__main__":
    ...
    obs_file = "/home/haoyang/ILAMB/ConfBMA/DATA/pr/pr_GPCC_1976_2005.nc"
    variable = "pr"
